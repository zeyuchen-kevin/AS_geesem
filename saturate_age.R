...
svVas     <- diag(svVa,ntv+1,ntv+1)

#Saturated Model
#Create Algebra for expected Mean Matrices
meanMZ = mxMatrix(type = "Full",nrow=1, ncol=ntv+1, free=T, values=svMe, labels=c("mMZ1","mMZ2","mMZ3"), name="meanMZ")
meanDZ = mxMatrix(type = "Full",nrow=1, ncol=ntv+1, free=T, values=svMe, labels=c("mDZ1","mDZ2","mDZ3"), name="meanDZ")

#Create Algebra for expected Variance/Covariance Matrices
covMZ = mxMatrix(type="Symm", nrow=ntv+1, ncol=ntv+1, free=T, values=svVas, labels=c("vMZ1","cMZ21","vMZ2","cMZ31","cMZ32","vMZ3"), name="covMZ")
covDZ = mxMatrix(type="Symm", nrow=ntv+1, ncol=ntv+1, free=T, values=svVas, labels=c("vDZ1","cDZ21","vDZ2","cDZ31","cDZ32","vDZ3"), name="covDZ")
...
SaturatedModel  <- mxModel( "ageSAT", modelMZ, modelDZ, fitML )
...
# Set Starting Values
svMe      <- 0            # start value for means
svVa      <- diag(0.5,ntv+1)          # start values for variances

# Algebra for expected Mean Matrices in MZ & DZ twins
meanMZ    <- mxMatrix( type="Full", nrow=1, ncol=ntv+1, free=TRUE,
                       values=svMe, labels=labFull("meMZ",1,ntv+1), name="expMeanMZ" )
meanDZ    <- mxMatrix( type="Full", nrow=1, ncol=ntv+1, free=TRUE,
                       values=svMe, labels=labFull("meDZ",1,ntv+1), name="expMeanDZ" )

# Algebra for expected Variance/Covariance Matrices in MZ & DZ twins
covMZ     <- mxMatrix( type="Symm", nrow=ntv+1, ncol=ntv+1, free=TRUE,
                       values=svVa, labels=labSymm("vaMZ",ntv+1), name="expCovMZ" )
covDZ     <- mxMatrix( type="Symm", nrow=ntv+1, ncol=ntv+1, free=TRUE,
                       values=svVa, labels=labSymm("vaDZ",ntv+1), name="expCovDZ" )
